...
def vmac_participant_match(superset_id, participant_index, sdx):
    ...
    vmac_addr = '{num:0{width}x}'.format(num=int(vmac_bitstring, 2), width=int(sdx.VMAC_size/4))
    vmac_addr = ':'.join([vmac_addr[i]+vmac_addr[i+1] for i in range(0, int(sdx.VMAC_size/4), 2)])
